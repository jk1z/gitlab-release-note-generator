...
describe("Gitlab adapter", () => {
    describe("#_decorateLinks", () => {
        ...
        test("should decorate links if link contains first, last, next and prev", async () => {
            ...
            const linkObj = Gitlab._decorateLinks(
            ...
            await linkObj.next();
            await linkObj.prev();
            expect(Gitlab.searchTagsByProjectId.mock.calls[0]).toEqual([
                "mockProjectId",
                { mockKey: "mockValue", page: "1", per_page: "20" }
            ]);
            expect(Gitlab.searchTagsByProjectId.mock.calls[1]).toEqual([
                "mockProjectId",
                { mockKey: "mockValue", page: "3", per_page: "20" }
            ]);
            expect(Gitlab.searchTagsByProjectId.mock.calls[2]).toEqual([
                "mockProjectId",
                { mockKey: "mockValue", page: "1", per_page: "20" }
            ]);
            jest.resetAllMocks();
